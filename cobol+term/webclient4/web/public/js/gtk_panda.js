...
$.widget('monsia.GtkFixed',$.monsia.GtkWidget,{
  ...
  _setOption: function(key,value) {
    ...
    case 'state':
      $.each(this._children,function(i,c) {
        var widget = $(c.widget);
        if (widget && widget[widget.attr("klass")]) {
          widget[widget.attr("klass")]("option",{state:value});
        }
      });
      break;
    }
  ...
  put: function(x,y,object) {
    ...
    this._children.push({x:x,y:y,object:object});
...
$.widget('monsia.GtkNumberEntry',$.monsia.GtkWidget,{
  ...
  serialize: function(data) {
    if (this.anonName) {
      data[this.anonName] = this.getValue() * 1.0;
    }
    return data;
  }
});
...
$.widget('monsia.GtkPandaTable',$.monsia.GtkWidget,{
        ...
        $(this.cells[i][j])
          ...
          .keydown(function(e) { //enter
            ...
            if (e.keyCode == 13 && 
              ...
              widget.options.cell_edited(widget.name,i,j,$(this).attr("value"));
              return;
            }
            if (e.keyCode == 38 && // up 
                e.shiftKey == false && 
    ...
    $.each(data,function(k,v) {
      ...
      case 'rowdata':
        ...
        for(var i=0;i<v.length;i++) {
          ...
          var bg = [];
          var cd = [];
          for(var j=0;j<r.__order.length;j++) {
            var colValue = r[r.__order[j]];
            $.each(colValue,function(cellKey,cellValue) {
              switch(cellKey) {
              case 'celldata':
                cd.push(cellValue);
                break;
  ...
  serialize: function(data) {
    ...
    if (this.cells) {
      for(var i=0;i<this.options.nrows;i++) {
        var j = 0;
        $.each(data.rowdata[i],function(colKey,colValue) {
          colValue.celldata = $(cells[i][j]).attr("value");
          j++;
        });
      }
    }
...
$.widget('monsia.GtkPandaCList',$.monsia.GtkWidget,{
  ...
  deserialize: function(data) {
    ...
    $.each(data,function(k,v) {
      ...
      case 'item':
        ...
        for(var i=0;i<v.length;i++) {
          var r = [];
          for(var j = 0; j<v[i].__order.length;j++) {
            r.push(v[i][v[i].__order[j]]);
          }
          rowdata.push(r);
        }
        break;
  ...
  serialize: function(data) {
    ...
    for(var i=0;i<slist.length;i++) {
      data[this.booldataName][i] = slist[i];
    }
    return data;
  }
});
